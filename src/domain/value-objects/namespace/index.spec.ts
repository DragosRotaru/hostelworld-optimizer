...
describe("name-space-string", () => {
  ...
  test("it encodes/decodes", () => {
    pipe("d243r3-4r3r3:r23r-f34r-ff43f", Codec.decode, result =>
      expect(isRight(result)).toBeTruthy()
    );
    pipe("2332d-d3-32:d:f3-32", Codec.decode, result =>
      expect(isRight(result)).toBeTruthy()
    );
    pipe("23-:d3-32df3-32", Codec.decode, result =>
      expect(isLeft(result)).toBeTruthy()
    );
    pipe("-34242e3-d23", Codec.decode, result =>
      expect(isLeft(result)).toBeTruthy()
    );
    pipe("-:", Codec.decode, result => expect(isLeft(result)).toBeTruthy());
    pipe("34242e::3-d23", Codec.decode, result =>
      expect(isLeft(result)).toBeTruthy()
    );
  });
});
